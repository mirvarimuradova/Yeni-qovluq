...
for (let i = 0; i < 3; i++) {
        ...
        let pattern = /\d/g;
        let result = name.slice(0, 1).match(pattern);
        while (name == "" || result) {


            name = prompt("adinizi duzgun daxil edin")
            result = name.slice(0, 1).match(pattern);

        }
        let surname = prompt(" soyadinizi daxil edin");
        result = surname.slice(0, 1).match(pattern);
        while (surname == "" || result) {
        ...
        age = parseInt(prompt(" yasinizi daxil edin"));

        let pattern1 = /^\d+$/;
        let resultAge = pattern1.test(age)
        while (!resultAge || age < 0) {
            age = parseInt(prompt(" yasinizi daxil edin"));
            resultAge = pattern1.test(age)
        }
        if (i == 0) {
